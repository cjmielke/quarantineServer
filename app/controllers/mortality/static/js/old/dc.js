...
dc.leaderBoard = function (parent, chartGroup) {
    ...
    function createElements(rows) {
        ...
        rowEnter.append("rect").attr("width", 0);
        rowEnter.append("image").attr("width", 0);
    ...
    function updateElements(rows) {
        ...
        var rect = rows.attr("transform",function (d, i) {
            ...
            .attr("height", height)
            .attr("fill", function (d) { return d3.scale.category20().range()[(d.key-1)%20];})
            .on("click", onClick)
            .classed("deselected", function (d) {
                return (_chart.hasFilter()) ? !isSelectedRow(d) : false;
            })
	...
	var img = rows.select("image")
		.attr("x", -height-3)
		.attr("y", 0)
		.attr("height", height).attr("width", height)
		.attr("xlink:href", function (d) { return _chart.title()(d); })
		.on("click", onClick)
		.attr("class", function (d, i) {
			return _rowCssClass + " _" + i;
		})
	...
	//dc.transition(img, _chart.transitionDuration()).attr("transform", translateX);
    ...
    function createLabels(rowEnter) {
        if (_chart.renderLabel()) {
            ...
            rowEnter.append("image").on("click", onClick);
        }
        if (_chart.renderTitleLabel()) {
            rowEnter.append("text")
                .attr("class", _titleRowCssClass)
